...
class TableCtrl extends Controller
{
    ...
    public function show($id)
    {
        ...
        $ip_address = $columnsValues['ip_address'];

        $ch = curl_init();

        curl_setopt($ch, CURLOPT_URL, "ip-api.com/json/" . $ip_address);
        ...
        if ($return['status'] == 'success') {
            $location = $return['city'] . ', ' . $return['country'];
        }
        else {
        ...
        $contents = View::make('pages.admin.record_show')->with($data);
